...
func genIncludeFiles() {
	const cmakefile = "./internal/protobuf-cpp-3.0.0-alpha-2/CMakeLists.txt"
	ss := parseCMakeListsTxt(cmakefile, "LIBPBC_SRC", ".cc")
	for i := 0; i < len(ss); i++ {
		relpath := ss[i][2:] // drop `./`
...
#include "./internal/protobuf-cpp-3.0.0-alpha-2/%s"
`, relpath,
		)), 0666)
